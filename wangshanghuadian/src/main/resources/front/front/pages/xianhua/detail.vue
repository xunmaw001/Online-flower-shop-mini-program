		...
		export default {
			...
			methods: {
				...
				async init() {
					...
					this.detail.xianhuaContent = this.detail.xianhuaContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					...
					if (res.data.list.length == 1) {
						...
						uni.showModal({
							...
							success: async function(res) {
								if (res.confirm) {
									await _this.$api.del('xianhuaCollection', JSON.stringify([xianhuaCollectionId]));
